...
- Name: TC1_SignUpTest.spec.js >> TEST#1: SignUp Test
- Location: tests\TC1_SignUpTest.spec.js:5:5
      ...
      - generic [ref=e4]:
        ...
        - generic [ref=e9]:
          - generic [ref=e10]:
            ...
            - textbox "Username:" [ref=e12]: princebabu225
          - generic [ref=e13]:
            - generic [ref=e14]: "Password:"
            - textbox "Password:" [ref=e15]: password@1232
        - generic [ref=e17]:
    ...
    - list [ref=e24]:
      ...
      - listitem [ref=e34]:
        - link "Log out" [ref=e35] [cursor=pointer]:
          - /url: "#"
      - listitem [ref=e36]:
        - link "Welcome princebabu225" [ref=e37] [cursor=pointer]:
          - /url: "#"
      - listitem
    - generic [ref=e39]:
      - list [ref=e40]:
...
```ts
  ...
  2  | import { PageObjectsManager } from '../Pages/PageObjectsManager';
  3  | const testData = require('../Utils/TestData.json')
  4  | const data = JSON.parse(JSON.stringify(testData))
  5  | test('TEST#1: SignUp Test', async ({ page }) => {
  6  |     const pageObjectsManager = new PageObjectsManager(page);
  7  |     const LoginPage = pageObjectsManager.getLoginPage();    
  8  |     await LoginPage.goToUrl(data.url);
  9  |     await LoginPage.openSignUpForm();
  10 |     await LoginPage.typeUserName_SignUp(data.userName);
  11 |     await LoginPage.typePassword_SignUp(data.password);
  12 |     const alertMessage = await LoginPage.clickSignUpButton();
> 13 |     expect(alertMessage).toBe(data.signUpSuccessMessage);
     |                          ^ Error: expect(received).toBe(expected) // Object.is equality
  14 | });
```